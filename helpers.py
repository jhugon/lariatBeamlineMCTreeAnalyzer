...
def plotManyHistsOnePlot(fileConfigs,histConfigs,canvas,treename,outPrefix="",outSuffix="Hist",nMax=sys.maxint):
  ...
  histConfig options:
    ...
    drawhlines: list of y locations to draw horizontal lines
    drawvlines: list of x locations to draw vertical lines
    stack: if you want hist to be in a hist stack
  """
  ...
  for fileConfig in fileConfigs:
    ...
    for histConfig in histConfigs:
      ...
      if "stack" in histConfig and histConfig["stack"]:
        stackHists.append(hist)
      else:
        hists.append(hist)
    canvas.SetLogy(logy)
    canvas.SetLogx(logx)
    histsForAxis = list(hists)
    histStack = root.THStack()
    stackSumHist = None
    if len(stackHists) > 0:
        for hist in reversed(stackHists):
            hist.SetFillColor(hist.GetLineColor())
            histStack.Add(hist)
            if stackSumHist is None:
                stackSumHist = hist.Clone(hist.GetName()+"_sumHist")
            else:
                stackSumHist.Add(hist)
        histsForAxis.append(stackSumHist)
    axisHist = makeStdAxisHist(histsForAxis,logy=logy,freeTopSpace=0.35,xlim=xlim,ylim=ylim)
    setHistTitles(axisHist,xtitle,ytitle)
    axisHist.Draw()
    for hlineY in hlineYs:
      hlines.append(drawHline(axisHist,hlineY))
    ...
    legOptions = ["lep"]*len(hists)+["F"]*len(stackHists)
    leg = drawNormalLegend(hists+stackHists,labels,legOptions,wide=True)
    drawStandardCaptions(canvas,caption,captionleft1=captionleft1,captionleft2=captionleft2,captionleft3=captionleft3,captionright1=captionright1,captionright2=captionright2,captionright3=captionright3,preliminaryString=preliminaryString)
    canvas.RedrawAxis()
